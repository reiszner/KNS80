var config = io.read_properties("path/to/kns80-config.xml");
...
setlistener(kns80_inner, func(inner) {
    if (inner.getValue()) {
        var pos = getprop("instrumentation/kns80/knob-inner-pos") + inner.getValue() * 15;
...
setlistener(kns80_outer, func(outer) {
    if (outer.getValue()) {
        var pos = getprop("instrumentation/kns80/knob-outer-pos") + outer.getValue() * 24;
        if (pos < 0  ) pos = pos + 360;
